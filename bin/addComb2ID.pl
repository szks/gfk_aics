...
my $time = "/usr/bin/time -f 'time(real,user,sys) = %e\t%U\t%S' ";
...
while(<IN>) {
    ...
    if ($junc11_strand eq "+") {
        ...
        my $ret = system($time . $GFK . "/samtools view " . $seq . " " . $junc11_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc." . $sample . ".sam");
        if ($ret != 0) {
    ...
    if ($junc21_strand eq "+") {
        ...
        my $ret = system($time . $GFK . "/samtools view " . $seq . " " . $junc21_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc." . $sample . ".sam");
        if ($ret != 0) {
            print STDERR "samtools error";
            exit $ret;
        }
...
system($time . "rm " . $tempDir . "/temp_junc." . $sample . ".sam");
